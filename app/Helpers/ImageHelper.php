...
class ImageHelper
{
    ...
    public static function saveAdImage($imagefile,$path)
    {
        $originalImage=$imagefile;
        $myImage = Image::make($originalImage);
        $myImage->resize(900,150);
        $originalPath = public_path().$path;
        $filename = rand(0,100).time().'.'.$originalImage->getClientOriginalExtension();
        $myImage->save($originalPath.$filename);
        return $path.$filename;
    ...
    public static function saveMImage($imagefile,$path)
    {
        $originalImage=$imagefile;
        $myImage = Image::make($originalImage);
        $myImage->resize(1024,1024);
    ...
    public static function saveFCImage($imagefile,$path)
    {
        $originalImage=$imagefile;
        $myImage = Image::make($originalImage);
        $myImage->resize(1024,1024);
    ...
    public static function saveBCImage($imagefile,$path)
    {
        $originalImage=$imagefile;
        $myImage = Image::make($originalImage);
        $myImage->resize(1024,1024);
    ...
    public static function saveSImage($imagefile,$path)
    {
        $originalImage=$imagefile;
        $myImage = Image::make($originalImage);
        $myImage->resize(1024,1024);
